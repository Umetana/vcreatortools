# Custom Base Template V2.8 Technical Spec

本ドキュメントは、`custom_base_template_V2_8_dev` の内部構造、VCT SDK V2対応、画面内設定パネルの仕様をまとめた技術メモです。
...
色設定のHEX入力欄には `settings/inline-color-picker/` の `VCTInlineColorPicker` を接続します。ピッカーは対象inputへ7文字のHEX値を設定し、標準の `input` イベントを発火します。設定保存、ライブプレビュー、BroadcastChannel同期は既存の設定パネル側が担当します。`BG_GLASS` のような透明度付き `rgba(...)` 設定はピッカー対象外です。
