...
createApp({
  setup() {
    ...
    const buildHearts = (comment) => {
      const heartList = [];
      const mode = C.HEART_MODE || 'off';

      if (mode !== 'always' && !(mode === 'gift' && comment.isSpecial)) {
        return heartList;
      }

      const count = C.HEART_COUNT || 5;
      const sizeMin = C.HEART_SIZE_MIN || 14;
      const sizeMax = C.HEART_SIZE_MAX || 24;
      const durationMin = C.HEART_DURATION_MIN || 2;
      const durationMax = C.HEART_DURATION_MAX || durationMin;
      const delayMax = C.HEART_DELAY_MAX || 0;
      const color = C.HEART_USE_USER_COLOR ? comment.colorStr : (C.HEART_COLOR || '#ff0000');
      ...
      for (let i = 0; i < count; i += 1) {
        heartList.push({
          ...
          dur: durationMin + Math.random() * Math.max(0, durationMax - durationMin),
    ...
    onMounted(() => {
      ...
      OneSDK.ready().then(() => {
        OneSDK.connect();
        console.log(`View Comment Heart: Ready (Stack: ${C.STACK_DIRECTION || 'up'})`);
      });
    });
