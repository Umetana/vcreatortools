(function (global) {
  ...
  const INFO_PRESETS_KEY = "vct_info_hud_info_presets";
  ...
  function loadInfoPresets() {
    try {
      const parsed = JSON.parse(localStorage.getItem(INFO_PRESETS_KEY) || "[]");
      return Array.from({ length: 3 }, (_, index) => {
        const messages = parsed?.[index];
        return Array.isArray(messages) ? messages.map((item) => String(item)) : null;
      });
    } catch (err) {
      console.warn("[VCT_InfoHUD] INFO presets parse failed.", err);
      return [null, null, null];
    }
  }

  function saveInfoPreset(index, messages) {
    const safeIndex = Math.max(0, Math.min(2, Number(index) || 0));
    const presets = loadInfoPresets();
    presets[safeIndex] = Array.isArray(messages) ? messages.map((item) => String(item)) : [];
    localStorage.setItem(INFO_PRESETS_KEY, JSON.stringify(presets));
  }
