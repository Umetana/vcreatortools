(() => {
  ...
  function buildIncomingSupportRecord(normalizedComment) {
    if (!window.VCT_CORE_RECORDS || typeof VCT_CORE_RECORDS.buildSupport !== "function") {
      return null;
    }

    return VCT_CORE_RECORDS.buildSupport(normalizedComment, {
  ...
  async function handleIncomingComment(rawComment) {
    const normalizedComment = window.VCT_SDK && typeof VCT_SDK.normalize === "function"
      ? VCT_SDK.normalize(rawComment, { includeRaw: true })
      : null;
    if (!normalizedComment) {
      return;
    }

    const record = buildIncomingSupportRecord(normalizedComment);
    if (!record) {
      return;
    }
  ...
  async function boot() {
    ...
    if (!window.VCT_SDK || typeof VCT_SDK.normalize !== "function") {
      throw new Error("VCT_SDK が読み込まれていません");
    }

    if (!window.VCT_CORE_RECORDS || typeof VCT_CORE_RECORDS.buildSupport !== "function") {
      throw new Error("VCT_CORE_RECORDS が読み込まれていません");
    }
