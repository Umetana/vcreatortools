# VCT_IDB wrapper: internal module (legacy spec v0.5.0)

## 1. 概要

`vct_idb.js` は、VCT 系テンプレート / CommentRaid 系テンプレートで共有する IndexedDB ラッパーです。

この仕様版 `v0.5.0` は、旧 `__shared` 側の IndexedDB wrapper 系統を引き継ぐものです。
`_vct_core` 本体の版数とは別系統で管理します。
...
- ライブラリ版数: `v0.5.0`
- `_vct_core` で実行中の `DB_VERSION`: `window.__VCT_IDB_CORE__.DB_VERSION` を正とする
- 旧 `__shared/vct_idb.js` 単体では `DB_VERSION: 4` 系

## 2. 導入方法

テンプレートの `index.html` では、`idb.min.js` の後、メインスクリプトより前に読み込みます。

`_vct_core` 内では以下の分割構成で読み込みます。

```html
<script src="./lib/idb/idb.min.js"></script>
<script src="./js/vct_idb_core.js"></script>
<script src="./js/vct_idb_common.js"></script>
<script src="./js/vct_idb.js"></script>
<script src="./main.js"></script>
```

旧 `__shared` 系の単体 `vct_idb.js` とは読み込み構成が異なります。
...
- `streamId` が渡された場合は `lastSeenStreamId` と `recentStreamIds` を更新
...
- **v0.5.1-dev**: `users.lastSeenStreamId` / `users.recentStreamIds` を追加。`saveUserProfile()` に `streamId` が渡された場合、直近5件の来訪stream履歴を保持する。
